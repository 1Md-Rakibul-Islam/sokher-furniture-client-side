...
const BannerItem = ({slide}) => {
    ...
    return (
        <div id={`slide${id}`} className="carousel-item w-full flex-col justify-center items-center">
        <div className='carousel-img mx-auto'>
            <img src={image} className="w-full" />
        </div>
        <div className='-mt-36 text-center'>
                <h1 classsName='text-3xl md:text-6xl font-bold'>
                    {title}
                </h1>
                <p className=''>
                    {Decription}
            ...
            <a href={`#slide${prev}`} className="text-4xl px-5 text-">❮</a> 
            <a href={`#slide${next}`} className="text-4xl px-5 text-">❯</a>
            </div>
        </div>
    </div> 
